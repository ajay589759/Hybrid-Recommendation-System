...
conn = sqlite3.connect('lung_cancer_app.db')
cursor = conn.cursor()

# Create table for storing user inputs and predictions
cursor.execute('''CREATE TABLE IF NOT EXISTS lung_cancer_records (
...
def load_and_preprocess_data():
    ...
    if "Name" in df.columns or "Surname" in df.columns:
...
def train_model():
    ...
    report = classification_report(y_test, y_pred, target_names=["No Lung Cancer", "Lung Cancer"])
...
if not os.path.exists('lung_cancer_model.pkl') or not os.path.exists('scaler.pkl') or not os.path.exists('accuracy.pkl'):
...
df = load_and_preprocess_data()
feature_cols = df.drop('Result', axis=1).columns
inputs = {}
...
if st.button("Predict"):
    ...
    cursor.execute('''INSERT INTO lung_cancer_records (features, prediction) VALUES (?, ?)''',
                   (str(inputs), result))
    conn.commit()
...
if st.button("View All Records"):
    cursor.execute("SELECT * FROM lung_cancer_records")
